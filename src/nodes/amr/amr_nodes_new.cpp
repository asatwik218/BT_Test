...
BT::NodeStatus TranslateAMR::tick() {
    ...
    int status = AMRApi::instance().translation(type, ip.value().c_str(), &params);
    if (status != 0) {
...
BT::NodeStatus RotateAMR::tick() {
    ...
    int status = AMRApi::instance().rotation(type, ip.value().c_str(), &params);
    if (status != 0) {
...
BT::NodeStatus GoToAMR::tick() {
    ...
    int status = AMRApi::instance().goTo(type, ip.value().c_str(), &params);
    if (status != 0) {
